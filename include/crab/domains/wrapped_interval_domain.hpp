...
EXIT_SIGNED_MUL:
        ...
        public:
            ...
            interval<Number> to_interval(bool is_signed) const {
                typedef interval<Number> interval_t;
                if (is_bottom()) {
                    return interval_t::bottom();
                } else if (is_top() || (cross_signed_limit() && is_signed) || (cross_unsigned_limit() && !is_signed)) {
                    return interval_t::top();
                } else {
                    if (is_signed) {
                        return interval_t(_start.get_signed_bignum(), _stop.get_signed_bignum());
                    } else {
                        return interval_t(_start.get_unsigned_bignum(), _stop.get_unsigned_bignum());
                    }
                }
            }
...
namespace crab {
    namespace domains {
        ...
        private:
            ...
            typedef enum {
                UNKNOWN_SIGNEDNESS, SIGNED, UNSIGNED
            } signedness_t;
            ...
            void rectify(const variable_t& v, signedness_t signedness) {
                ...
                if (signedness == SIGNED) {
                    ...
                    crab::outs() << " unknown unsignedness: \n";
                });
        ...
        public:
            ...
            wrapped_numerical_domain_t& first() {
            ...
            void operator+=(linear_constraint_system_t csts) {
                ...
                for (auto c : csts) {
                    ...
                    for (auto const& v : c.variables()) {
                        ...
                        // _product.second() += _product.first()[v]; //injection of wrapped interval constraints
                    }

                    // add constraint in the unsound numerical domain only if
                    // expr(c) cannot overflow.
                    linear_expression_t lexpr = c.expression();
                    if (!may_overflow(lexpr, signedness)) {
                        non_overflow_csts += c;
                    } else {
                        //overflow case
                        wrap_expr(lexpr, signedness);
                        non_overflow_csts += c;
                    }
                }
            ...
            void wrap_expr(linear_expression_t lexpr, signedness_t signedness) {
                variable_t v_new = create_fresh_wrapped_int_var(lexpr);
                _product.first() += (lexpr == v_new);
                _product.second() += (lexpr == v_new);
                rectify(v_new, signedness);
                //_product.second() += _product.first()[v_new];
                _product.first() -= v_new;
                _product.second() -= v_new;
            }
            ...
            variable_t create_fresh_wrapped_int_var(linear_expression_t lexpr) {
                ...
                variable_t var = *(lexpr.variables().begin());
                variable_t v_new(var.name().get_var_factory().get(), var.get_type(), var.get_bitwidth());
                return v_new;
            }

            void set(variable_t x, interval_t intv) { // domain_product2 does not define set method
                _product.first().set(x, intv);
                _product.second().set(x, intv);
            }

            interval_t operator[](variable_t v) { // domain_product2 does not define [] method
